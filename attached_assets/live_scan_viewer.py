...
class TileScanViewer:
    def __init__(self, master, full_scan_path, segmentation_history_dir, tile_info_path, update_interval_ms=1000):
        ...
        self.segmentation_history_dir = segmentation_history_dir # Directory for segmentation images
        self.tile_info_path = tile_info_path
        self.update_interval_ms = update_interval_ms

        self.master.title("Live Tile Scan Viewer")
        self.master.geometry("1200x850") # Increased height for buttons
        ...
        self.nav_status_label = ttk.Label(self.controls_frame, text="LIVE", anchor=tk.CENTER, width=20) # Increased width
    ...
    def load_full_scan_image(self):
        if not os.path.exists(self.full_scan_path):
            ...
            if self.full_scan_canvas.winfo_width() > 1 and self.full_scan_canvas.winfo_height() > 1: # Check if canvas is ready
    ...
    def draw_tile_highlight(self, tile_coords): # tile_coords = (y,x,h,w)
    ...
    def update_navigation_status(self):
        ...
        if self.is_live_tracking:
            self.nav_status_label.config(text=f"LIVE ({self.current_history_index + 1}/{len(self.segmentation_history)})")
        else:
            self.nav_status_label.config(text=f"History ({self.current_history_index + 1}/{len(self.segmentation_history)})")

        self.prev_button.config(state=tk.NORMAL if self.current_history_index > 0 else tk.DISABLED)
        self.next_button.config(state=tk.NORMAL if self.current_history_index < len(self.segmentation_history) - 1 else tk.DISABLED)
    ...
    def show_next_segmentation(self):
        if self.current_history_index < len(self.segmentation_history) - 1:
            self.current_history_index += 1
            if self.current_history_index == len(self.segmentation_history) - 1:
                self.is_live_tracking = True # Resume live tracking if we reach the latest
            else:
                self.is_live_tracking = False
            self.display_current_segmentation()
    ...
    def update_views(self):
        updated = self.update_tile_info()
        if updated or (self.is_live_tracking and self.segmentation_history and self.current_history_index != len(self.segmentation_history) -1 ):
            # If live tracking and new history item added, or if history was updated and we are live tracking
            if self.is_live_tracking and self.segmentation_history:
                 self.current_history_index = len(self.segmentation_history) - 1
            self.display_current_segmentation()

        self.update_navigation_status() # Always update nav status
        self.master.after(self.update_interval_ms, self.update_views)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Live Tile Scan Viewer with History for Segmentation Script.")
    parser.add_argument("full_scan_image_path", help="Path to the full tile scan image (e.g., whole_slide.tif).")
    parser.add_argument("segmentation_history_dir", help="Directory where sequentially named segmentation images are stored.")
    parser.add_argument("tile_info_path", help="Path to a text file containing current tile coordinates, scan dimensions, and segmentation image filename (e.g., current_tile_info.txt).")
    parser.add_argument("--interval", type=int, default=1, help="Update interval in seconds. Default: 1s")

    args = parser.parse_args()

    if not os.path.isdir(args.segmentation_history_dir):
        print(f"Error: Segmentation history directory not found: {args.segmentation_history_dir}")
        exit(1)
