...
# --- Helper Functions --- (get_cell_outlines, segment_chunk remain the same)
...
# --- Visualization Functions --- (create_rgb_roi, visualize_roi_combined remain the same, but calls were fixed)
def create_rgb_roi(image_data_stack, y_start, y_end, x_start, x_end, vis_ch_indices=[0, 1]):
    ...
    if len(vis_ch_indices) > 2: # R, G, B
        rgb_image[..., 0] = roi_ch_r_norm # vis_ch_indices[0] -> R
        rgb_image[..., 1] = roi_ch_g_norm # vis_ch_indices[1] -> G
        rgb_image[..., 2] = roi_ch_b_norm # vis_ch_indices[2] -> B
    elif len(vis_ch_indices) > 1: # G, B
        rgb_image[..., 1] = roi_ch_g_norm # vis_ch_indices[0] -> G
        rgb_image[..., 2] = roi_ch_b_norm # vis_ch_indices[1] -> B
    elif len(vis_ch_indices) > 0: # G (grayscale)
        rgb_image[..., 0] = roi_ch_g_norm # vis_ch_indices[0] -> R
        rgb_image[..., 1] = roi_ch_g_norm # vis_ch_indices[0] -> G
        rgb_image[..., 2] = roi_ch_g_norm # vis_ch_indices[0] -> B
...
def visualize_roi_combined(image_data_stack, roi_position, roi_size,
                           ...
                           original_bg_channels_for_title=[0,1]): # These are original TIFF channel numbers
    logger = logging.getLogger(__name__)
    ...
    if actual_roi_height <= 0 or actual_roi_width <= 0:
        ...
        return None
    try:
        # vis_ch_indices for create_rgb_roi are 0-indexed relative to image_data_stack
    ...
    ch_map_str_list = []
    # original_bg_channels_for_title refers to the original TIFF channels used to create image_data_stack
    # vis_ch_indices refers to how those channels (now in image_data_stack) are mapped to R,G,B
    # For title, we use original_bg_channels_for_title
    if len(original_bg_channels_for_title) == 1: # Grayscale mapped to G
        ch_map_str_list.append(f"Display: G=OrigCh{original_bg_channels_for_title[0]}")
    elif len(original_bg_channels_for_title) == 2: # G, B
        ch_map_str_list.append(f"Display: G=OrigCh{original_bg_channels_for_title[0]}, B=OrigCh{original_bg_channels_for_title[1]}")
    elif len(original_bg_channels_for_title) >= 3: # R, G, B
...
def process_large_image(
    ...
    nuclei_cellprob_threshold=-1.5, # Initial cellprob threshold
    nuclei_channels=[2,1],
    # Adaptive Nuclei Segmentation parameters (now for cellprob, DECREASING)
    enable_adaptive_nuclei=False,
    nuclei_adaptive_cellprob_lower_limit=-6.0, # Min cellprob (more negative)
    nuclei_adaptive_cellprob_step_decrement=0.5, # Step to decrease cellprob
    ...
    vis_bg_channel_indices=[0,1], # Original TIFF 0-indexed channels for BG
                ...
                if enable_adaptive_nuclei and num_cells_found > 0:
                    ...
                    if trigger_adaptive:
                        ...
                        for attempt in range(nuclei_max_adaptive_attempts):
                            adaptive_nuclei_cellprob_current -= nuclei_adaptive_cellprob_step_decrement # DECREASE cellprob
                ...
                if live_update_image_path:
                    try:
                        ...
                        if target_channel_source.ndim == 3:
                            for ch_idx_orig_tiff in vis_bg_channel_indices: # These are original TIFF indices
                                ...
                                else: # Fallback if original index is out of bounds for the chunk's available channels
                        ...
                        if not live_chunk_display_stack_list:
                            ...
                            live_fig = visualize_roi_combined(
                                ...
                                vis_ch_indices=list(range(live_chunk_display_data.shape[0])), # Use all channels from the prepared stack
                                original_bg_channels_for_title=vis_bg_channel_indices, # Pass original TIFF indices for title
                                figsize=(8,8)
                            )
                            if live_fig:
            ...
            if num_to_sel > 0:
                ...
                try:
                    with tifffile.TiffFile(image_path) as tiff_vis:
                        ...
                        if s0_ndim < 3 and not (s0_ndim == 2 and len(vis_bg_channel_indices)==1 and vis_bg_channel_indices[0]==0) : # Allow 2D if only asking for channel 0
                            ...
                            channel_axis_for_vis = 0 if s0_ndim == 3 else (1 if s0_ndim == 4 else -1) # -1 for 2D case
                            ...
                            if visualize:
                                ...
                                for ch_idx_orig in vis_bg_channel_indices:
                                    if s0_ndim == 2: # Handle 2D image
                                        ...
                                        if s0_ndim == 4: key_list[0] = 0 # Assume T=0 for 4D
            ...
            if visualize and vis_img_stack_bg is not None:
                for y_s_chunk_global, x_s_chunk_global, chunk_id_str in selected_vis_chunks:
                    ...
                    fig = visualize_roi_combined(
                        ...
                        vis_ch_indices=list(range(vis_img_stack_bg.shape[0])), # Use all channels from the prepared stack
                        original_bg_channels_for_title=vis_bg_channel_indices # Pass original TIFF indices for title
                        # figsize will use its default (10,10)
                    )
                    if fig:
                        try:
