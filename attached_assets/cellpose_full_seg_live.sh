...
PYTHON_SCRIPT="/vast/projects/BCRL_Multi_Omics/segmentation_pipeline/cellpose_wsi_seg_for_viewer.py" # Your Python script
...
NUCLEI_DIAMETER=80.0
NUCLEI_FLOW_THRESH=0.6
NUCLEI_CELLPROB_THRESH=-1.2
NUCLEI_CHANNELS="2 0"
...
ENABLE_SUMMARY_VISUALIZATION="true"
NUM_SUMMARY_VIS_CHUNKS=10
VIS_ROI_Y=2024
VIS_ROI_X=2024
VIS_BG_CHANNELS_FOR_PLOTS="0 1"

# --- Live Update Parameters ---
ENABLE_LIVE_UPDATE="true" # "true" or "false"
# UPDATED: Define directory for live segmentation images
LIVE_SEGMENTATIONS_SUBDIR="live_segmentations"
LIVE_UPDATE_IMAGE_DIR="${BASE_OUTPUT_DIR}/${LIVE_SEGMENTATIONS_SUBDIR}"

# Define path for the tile info file for the live scan viewer
...
if [ "$ENABLE_SUMMARY_VISUALIZATION" = "true" ]; then
    mkdir -p "$VIS_OUTPUT_DIR" # Ensure summary vis dir exists
    CMD+=" --visualize"
...
if [ "$ENABLE_LIVE_UPDATE" = "true" ]; then
    mkdir -p "$LIVE_UPDATE_IMAGE_DIR" # Ensure live update image directory exists
    CMD+=" --live_update_image_dir \"$LIVE_UPDATE_IMAGE_DIR\"" # UPDATED argument
    CMD+=" --tile_info_file_for_viewer \"$TILE_INFO_FILE_PATH\""

    if [ "$ENABLE_SUMMARY_VISUALIZATION" != "true" ]; then
        # Pass vis_bg_channel_indices if live update is on, even if summary viz is off
        CMD+=" --vis_bg_channel_indices \"$VIS_BG_CHANNELS_FOR_PLOTS\""
    fi
fi
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ "$ENABLE_LIVE_UPDATE" = "true" ]; then
        echo "Live update image directory: $LIVE_UPDATE_IMAGE_DIR" # UPDATED message
